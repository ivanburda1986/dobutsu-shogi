...
export const isStartingPlayerDetermined = (
  startingPlayer: string | undefined
): boolean => {
  return !!startingPlayer;
};

export const haveBothPlayersJoined = (
...
export const determineStartingPlayer = (
  ...
  if (
    haveBothPlayersJoined(creatorId, opponentId) &&
    !isStartingPlayerDetermined(startingPlayer)
  ) {
    ...
    updateGame({
      id: gameId!,
      updatedDetails: {
        startingPlayer: whoShouldStart,
        currentPlayerTurn: whoShouldStart,
      },
    });
  }
};
...
export const shouldSaveLatestRoundMovementHash = (
  moveRepresentations: string[],
  latestRoundMoveHash: string | undefined
): boolean => {
  return (
    !!latestRoundMoveHash &&
    moveRepresentations[moveRepresentations.length - 1] !== latestRoundMoveHash
  );
};
...
export const createLatestRoundMoveHash = (
  hashInput: BothPlayerHashInput
): string | undefined => {
  const { player1, player2 } = hashInput;
  const latestRoundMoveHash = (
    player1.movingPlayerId.charAt(0) +
    player1.type.charAt(0) +
    player1.fromCoordinates +
    player1.targetCoordinates +
    player2.movingPlayerId.charAt(0) +
    player2.type.charAt(0) +
    player2.fromCoordinates +
    player2.targetCoordinates
  ).toLowerCase();
...
export const areSufficientMoveRecordsAvailable = (
  gameData: DocumentData | undefined
): boolean => {
  return !!(
    gameData &&
    gameData.moves.length >= 2 &&
    gameData.moves.length % 2 === 0
  );
};
...
export const createAndStoreLastRoundMoveHash = (
  ...
  const latestRoundMoveHash = createLatestRoundMoveHash({
    player1: player1LatestMove,
    player2: player2LatestMove,
  });

  const moveRepresentations = gameData?.moveRepresentations;
  if (
    shouldSaveLatestRoundMovementHash(moveRepresentations, latestRoundMoveHash)
  ) {
    let updatedMoveRepresentations = [
      ...moveRepresentations,
      latestRoundMoveHash,
    ];
    updateGame({
...
export const isTieEvaluation = (
  ...
  const moveRepresentations = gameData?.moveRepresentations;
  const isMinimumCountOfMovementsAvailableForTieToOccur =
    moveRepresentations.length >= 6;
  ...
  const lastMinusThreeRound =
    moveRepresentations[moveRepresentations.length - 3];
  const lastMinusFiveRound =
    moveRepresentations[moveRepresentations.length - 5];

  return (
    lastRound === lastMinusThreeRound &&
    lastMinusThreeRound === lastMinusFiveRound
  );
};

export const isThisPlayerOpponent = (
  creatorId: string,
...
export const increaseTieStatsCountForBothPlayers = (
  player1Id: string,
  player2Id: string
): void => {
...
export function increaseUserWinStats(conqueredPlayerId: string | undefined) {
...
export function increaseUserLossStats(conqueredPlayerId: string | undefined) {
  getSingleUserStats({ userId: conqueredPlayerId! }).then((serverStats) =>
    updateUserStats({
      userId: conqueredPlayerId!,
